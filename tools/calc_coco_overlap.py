...
def iou(box1, box2):
    ...
    inter = max(min(box1[2], box2[2]) - max(box1[0], box2[0]) + 1, 0) * max(
        min(box1[3], box2[3]) - max(box1[1], box2[1]) + 1, 0
    )
        ...
        for ann in anns:
            ...
            for c in centers:
                if (
                    ...
                    and center[1] == c[1]
                    and center[2] == c[2]
                    and iou(_coco_box_to_bbox(bbox), _coco_box_to_bbox(c[3])) < 2
                ):  # 0.5:
                    cnt += 1
                    if DEBUG:
                        ...
                        cv2.rectangle(
                            img, (x1, y1), (x2, y2), (255, 0, 0), 2, cv2.LINE_AA
                        )
                        x1, y1 = (
                            int(center[3][0]),
                            int(center[3][1]),
                        )
                        x2, y2 = int(center[3][0] + center[3][2]), int(
                            center[3][1] + center[3][3]
                        )
                        cv2.rectangle(
                            img, (x1, y1), (x2, y2), (0, 0, 255), 2, cv2.LINE_AA
                        )
                        cv2.imshow("img", img)
                        cv2.waitKey()
            centers.append(center)
    ...
    for ind, img_id in enumerate(images):
        ...
        for i in range(w // stride):
            for j in range(h // stride):
                ct = np.array([i * stride, j * stride], dtype=np.float32).reshape(
                    1, 1, 2
                )
