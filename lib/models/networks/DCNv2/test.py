...
def check_gradient_dconv_double():
    ...
    offset = torch.randn(
        N, deformable_groups * 2 * kW * kH, inH, inW, dtype=torch.float64
    ).cuda()
    # offset.data.zero_()
    # offset.data -= 0.00001
    ...
    mask = torch.rand(
        N, deformable_groups * 1 * kW * kH, inH, inW, dtype=torch.float64
    ).cuda()
    # mask.data.zero_()
    ...
    func = DCNv2Function(
        stride=1, padding=1, dilation=1, deformable_groups=deformable_groups
    )

    print(
        gradcheck(
            func, (input, offset, mask, weight, bias), eps=1e-6, atol=1e-5, rtol=1e-3
        )
    )
...
def check_gradient_dconv():
    ...
    print(
        gradcheck(
            func, (input, offset, mask, weight, bias), eps=1e-3, atol=1e-3, rtol=1e-2
        )
    )
...
def check_gradient_dpooling():
    ...
    offset.requires_grad = True
    print(
        "check_gradient_dpooling", gradcheck(dpooling, (input, rois, offset), eps=1e-4)
    )
