    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            ...
            if opt.dep_weight > 0:
                dep_loss += (
                    self.crit_reg(
                        output["dep"], batch["reg_mask"], batch["ind"], batch["dep"]
                    )
                    / opt.num_stacks
                )
            if opt.dim_weight > 0:
            ...
            if opt.reg_bbox and opt.wh_weight > 0:
                wh_loss += (
                    self.crit_reg(
                        output["wh"], batch["rot_mask"], batch["ind"], batch["wh"]
                    )
                    / opt.num_stacks
                )
            if opt.reg_offset and opt.off_weight > 0:
                off_loss += (
                    self.crit_reg(
                        output["reg"], batch["rot_mask"], batch["ind"], batch["reg"]
                    )
                    / opt.num_stacks
                )
        loss = (
            opt.hm_weight * hm_loss
            + opt.dep_weight * dep_loss
            + opt.dim_weight * dim_loss
...
class DddTrainer(BaseTrainer):
    ...
    def debug(self, batch, output, iter_id):
        ...
        for i in range(1):
            ...
            img = ((img * self.opt.std + self.opt.mean) * 255.0).astype(np.uint8)
