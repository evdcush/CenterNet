...
class MultiPoseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        dense_kps_mask = np.zeros(
            (num_joints, output_res, output_res), dtype=np.float32
        )
        ...
        for k in range(num_objs):
            ...
            if (h > 0 and w > 0) or (rot != 0):
                ...
                hp_radius = (
                    self.opt.hm_gauss if self.opt.mse_loss else max(0, int(hp_radius))
                )
                for j in range(num_joints):
                    if pts[j, 2] > 0:
                        ...
                        if (
                            pts[j, 0] >= 0
                            and pts[j, 0] < output_res
                            and pts[j, 1] >= 0
                            and pts[j, 1] < output_res
                        ):
                            kps[k, j * 2 : j * 2 + 2] = pts[j, :2] - ct_int
                            kps_mask[k, j * 2 : j * 2 + 2] = 1
                            pt_int = pts[j, :2].astype(np.int32)
                            hp_offset[k * num_joints + j] = pts[j, :2] - pt_int
                            hp_ind[k * num_joints + j] = (
                                pt_int[1] * output_res + pt_int[0]
                            )
        ...
        if self.opt.debug > 0 or not self.split == "train":
            gt_det = (
                ...
                if len(gt_det) > 0
                else np.zeros((1, 40), dtype=np.float32)
            )
